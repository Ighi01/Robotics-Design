...
class Arm:
    ...
    def raise_percent(self, percent: int, delay: int, velocity: int, curve: Curve = Curve.LINEAR):
        assert 0 <= percent <= 100
        assert 0 <= delay
        assert 0 <= velocity <= self.max_velocity
        self.servo.add_movement(int(self.max_angle * percent / 100), delay, velocity, curve)

    def raise_full(self, delay: int, velocity: int, curve: Curve = Curve.LINEAR):
        self.raise_percent(100, delay, velocity, curve)

    def lower(self, delay: int, velocity: int, curve: Curve = Curve.LINEAR):
        self.raise_percent(0, delay, velocity, curve)

    def raise_half(self, delay: int, velocity: int, curve: Curve = Curve.LINEAR):
